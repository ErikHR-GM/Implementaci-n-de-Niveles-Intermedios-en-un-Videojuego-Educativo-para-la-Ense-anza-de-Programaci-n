...
if (place_meeting(x, y, obj_zona_resp_cpp)) {
    if (texto_respuesta == obj_control_cpp.respuesta_correcta) {
        show_message("Respuesta correcta");
        
        // Sumar 10 puntos directamente al global
        global.puntaje += 10;

        obj_control_cpp.indice_pregunta += 1;

        if (obj_control_cpp.indice_pregunta < array_length(obj_control_cpp.preguntas)) {
            with (obj_control_cpp) {
                cargar_pregunta_cpp();
            }
        }
