...
if (place_meeting(x, y, obj_zona_resp_c5)) {
    if (texto_respuesta == obj_control_c5.respuesta_correcta) {
        show_message("Respuesta correcta");
        
        // Sumar 10 puntos directamente al global
        global.puntaje += 10;

        obj_control_c5.indice_pregunta += 1;

        if (obj_control_c5.indice_pregunta < array_length(obj_control_c5.preguntas)) {
            with (obj_control_c5) {
                cargar_pregunta_c5();
            }
        }
